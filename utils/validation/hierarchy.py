...
def path_based_similarities(pred: AbstractDocx, ground_truth: AbstractDocx) -> dict[str, float]:
	...
	def get_paths(block: Block, curr_path: Optional[tuple[int]]=None) -> set[tuple[int]]:
		if curr_path is None:
			curr_path: tuple[int] = []
		path: tuple[int] = curr_path + [block.id]

		paths: set[tuple[int]] = {tuple(path)}
	...
	pred_paths: set[tuple[int]] = get_paths(pred.document_root)
	ground_truth_paths: set[tuple[int]] = get_paths(ground_truth.document_root)

	true_positives: set[tuple[int]] = pred_paths & ground_truth_paths
	tp: int = len(true_positives)
	false_positives: set[tuple[int]] = pred_paths - ground_truth_paths
	fp: int = len(false_positives)
	false_negatives: set[tuple[int]] = ground_truth_paths - pred_paths
	...
	def least_common_prefix(pred_path: tuple[int], ground_truth_path: tuple[int]) -> float:
		lcp: tuple[int] = [x for x, y in takewhile(lambda pair: pair[0] == pair[1], zip(pred_path, ground_truth_path))]

		return len(lcp)/len(ground_truth_path)
	
	def least_common_suffix(pred_path: tuple[int], ground_truth_path: tuple[int]) -> float:
		lcs: tuple[int] = [
